...
async def create_tfr(request: TFRCreate, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # Signal COMTRADE recording start (IOA 1500=1, 1501=0)
        ...
        # Add 1 second for processing time
        recording_duration = (tfr_detail.duration_ms / 1000.0) + 5.0 + 5.0 + 1.0
        ...
        async def auto_complete_recording():
            ...
            logger.info(f"Auto-completed COMTRADE recording for TFR {tfr_id} after {recording_duration:.1f}s")
...
async def export_tfr_comtrade(
    ...
    Signals COMTRADE availability via IEC 104 spontaneous transmission.
    """
    tfr = db.get_tfr(tfr_id)
    ...
    try:
        ...
        # Signal COMTRADE recording complete (IOA 1500=0, 1501=1, 1502=1)
        iec104_server.signal_comtrade_recording_complete(tfr_id)
